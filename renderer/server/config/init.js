...
const Notebook = require('../api/notebook/notebook.model');
const Session = require('../api/session/session.model');
const Settings = require('../api/settings/settings.model');
const Connections = require('../api/connections/connection.model');
  ...
  return new Promise((resolve, reject) => {
    Hashtags.find({}, (err, hashtags) => {
      ...
      if (!hashtags.length) {
        ...
        fs.readFile(path.join(app.getAppPath(), 'hashtags.json'), 'utf8', (err, contents) => {
          if (err) {
